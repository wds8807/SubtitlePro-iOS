...
class Subtitles {
	...
	static func parseSubRip(_ payload: String) -> [String: Any] {
	...
	///   - time: Time
	/// - Returns: String
	static func searchSubtitles(_ payload: [String: Any], at time: TimeInterval) -> String {
		
		//guard let values = payload?.values else { return nil }
		guard let results = Array(payload.values) as? [[String: Any]] else { return "" }
		
		let filteredResults = results.filter { (result) -> Bool in
			if let fromTime = result["from"] as? Double, let toTime = result["to"] as? Double {
				return time > fromTime && time < toTime
			}
			return true
		}
		guard let result = filteredResults.first else { return "" }
